...
public:
    ...
    R1CSProof prove(Transcript& transcript) {
        ...
        // Try GPU path: pass y_inv_n directly so GPU can compute H_prime
        // from pre-decoded base H generators (no rist_decode on hot path)
        if (!try_gpu_ipp_prove(transcript, Q, l_eval, r_eval, y_inv_n, N, proof.ipp)) {
            // CPU fallback: compute H_prime from compressed points
            static int cpu_ipa_count = 0;
            static auto cpu_first = std::chrono::steady_clock::now();
            ...
            double total = std::chrono::duration<double, std::milli>(cpu_end - cpu_first).count();
            printf("  [CPU IPA #%d] n=%zu, %.0fms (total %.1fs)\n",
                   cpu_ipa_count, N, ms, total / 1000.0);
            fflush(stdout);
        }
        return proof;
    }
